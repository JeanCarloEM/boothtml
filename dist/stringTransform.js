      ...
      ((checker, process) => {
        ...
        (process, clbackCheck) => {
          return n.match(regex) && c < m
            ? n
                .replaceAsync(regex, async (m, ctt, oset, input_string) => {
                  if (++c > m) {
                    ER("Over processing.");
          ...
          return new Promise((R1, RJ1) => {
            ...
            ((make, logic) => {
              ...
              (X, K, clbackLogic) => {
                ...
                if (mt) {
                  return ("${" + mt.groups.var.trim() + "}")
                    .transform(regex, src, defkey, ukn, filter)
                    .then((z) => {
                      z = z.trim().toLowerCase();

                      if (z == "true" || (isFinite(z) && parseFloat(z) > 0)) {
                        return clbackLogic(
                          mt.groups.true ? mt.groups.true : "",
                        );
                      } else if (
                        z == "false" ||
                        (isFinite(z) && parseFloat(z) == 0)
                      ) {
                        return clbackLogic(
                          mt.groups.false ? mt.groups.false : "",
                        );
                      }
